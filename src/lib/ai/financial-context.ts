...
const CACHE_MAX_SIZE = 100; // Max entries to prevent unbounded growth
...
export async function buildFinancialContext(userId: string): Promise<string> {
  ...
  const [
    ...
    supabase
      .from('goals')
      .select(
        'monthly_savings_target, retirement_age_target, streak_months, level, xp, total_challenges_completed',
      )
      .eq('user_id', userId)
      .single(),
    supabase
  ...
  if (prevExpenses > 0) {
    const expenseChange =
      expenses > 0 ? Math.round(((expenses - prevExpenses) / prevExpenses) * 100) : 0;
    lines.push(
      `Despesas mês anterior: ${formatCurrency(prevExpenses)} (${expenseChange > 0 ? '+' : ''}${expenseChange}% vs atual)`,
    );
  ...
  if (budgets.length > 0) {
    ...
    for (const b of budgets) {
      ...
      lines.push(
        `- ${catName}: ${formatCurrency(spent)}/${formatCurrency(limit)} (${pct}%) [${status}]`,
      );
    }
  }
  ...
  if (recurring.length > 0) {
    ...
    for (const r of recurring) {
      ...
      lines.push(
        `- ${sanitize(r.merchant)}: ${formatCurrency(Number(r.amount))}/mês (${typeLabel})`,
      );
    }
  }
  ...
  if (goals?.monthly_savings_target) {
    ...
    const savingsProgress =
      savingsTarget > 0 ? Math.round((currentSavings / savingsTarget) * 100) : 0;
    lines.push('', '--- Metas e Gamificação ---');
    lines.push(`Meta mensal de economia: ${formatCurrency(savingsTarget)}`);
    lines.push(
      `Economia atual no mês: ${formatCurrency(currentSavings)} (${Math.min(savingsProgress, 100)}%)`,
    );
    if (goals.level) lines.push(`Nível: ${goals.level} (${goals.xp || 0} XP)`);
    if (goals.streak_months)
      lines.push(`Sequência: ${goals.streak_months} meses consecutivos atingindo a meta`);
    if (goals.total_challenges_completed)
      lines.push(`Desafios completados: ${goals.total_challenges_completed}`);
  }
  ...
  if (contextCache.size >= CACHE_MAX_SIZE) {
    ...
    for (const [key, val] of contextCache) {
      if (val.expiresAt <= now) contextCache.delete(key);
    }
    // If still at max, remove oldest entry (first inserted)
    if (contextCache.size >= CACHE_MAX_SIZE) {
